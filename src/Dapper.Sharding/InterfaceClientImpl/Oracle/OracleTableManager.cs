...
namespace Dapper.Sharding
{
    internal class OracleTableManager : ITableManager
    {
        ...
        public override async Task CreateIndexAsync(string name, string columns, IndexType indexType)
        {
            if (indexType == IndexType.Normal)
            {
               await DataBase.ExecuteAsync($"CREATE INDEX {DataBase.Client.Config.UserId.ToUpper()}.\"{Name}_{name}\" ON {DataBase.Client.Config.UserId.ToUpper()}.\"{Name.ToUpper()}\" ({columns})");
            }
            else if (indexType == IndexType.Unique)
            {
               await DataBase.ExecuteAsync($"CREATE UNIQUE INDEX {DataBase.Client.Config.UserId.ToUpper()}.\"{Name}_{name}\" ON {DataBase.Client.Config.UserId.ToUpper()}.\"{Name.ToUpper()}\" ({columns})");
            }
        }
        ...
        public override async Task AddColumnAsync(string name, Type t, double length = 0, string comment = null)
        {
            ...
            await DataBase.ExecuteAsync($"ALTER TABLE {DataBase.Client.Config.UserId.ToUpper()}.{Name.ToUpper()} ADD({name.ToUpper()} {dbType})");
        ...
        {
           await DataBase.ExecuteAsync($"ALTER TABLE {Name.ToUpper()} DROP COLUMN {name.ToUpper()}");
        }
...
and col.table_name = '{Name.ToUpper()}'";
            ...
            IEnumerable<dynamic> data = await DataBase.QueryAsync(sql);
            IEnumerable<string> nameList = data.Select(s => (string)s.INDEX_NAME).Distinct();
            var list = new List<IndexEntity>();
            foreach (var n in nameList)
            {
