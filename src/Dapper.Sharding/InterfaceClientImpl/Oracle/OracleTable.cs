...
namespace Dapper.Sharding
{
    internal class OracleTable<T> : ITable<T> where T : class
    {
        ...
        public override IEnumerable<dynamic> GetByIdsForUpdateDynamic(object ids, string returnFields = null)
        {
            ...
            var dpar = new DynamicParameters();
            dpar.Add(":ids", ids);
            return DpEntity.Query($"SELECT {returnFields} FROM {Name} WHERE {SqlField.PrimaryKey} IN :ids FOR UPDATE", dpar);
        }
        ...
        public override dynamic GetByWhereFirstDynamic(string where, object param = null, string returnFields = null)
        {
            ...
            return DpEntity.QueryFirstOrDefault($"SELECT {returnFields} FROM {Name} {where}", param);
        }
