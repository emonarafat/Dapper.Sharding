...
namespace Dapper.Sharding
{
    internal class PostgreTableManager : ITableManager
    {
        ...
        public override async Task AddColumnAsync(string name, Type t, double length = 0, string comment = null)
        {
            ...
            {
                await DataBase.ExecuteAsync($"ALTER TABLE {Name} ADD COLUMN {name} {dbType};COMMENT ON COLUMN {Name.ToLower()}.{name.ToLower()} IS '{comment}'");
            }
        }
...
order by
    t.relname,
    i.relname;";
            var data1 = await DataBase.QueryAsync(sql1);
            var data2 = await DataBase.QueryAsync(sql2);
            var list = new List<IndexEntity>();
            foreach (var row in data2)
            {
                var model = new IndexEntity();
...
where table_schema='public' and table_name=current_setting('myapp.name') order by ordinal_position asc";
            ...
            IEnumerable<dynamic> data = await DataBase.QueryAsync(sql1);
            IEnumerable<dynamic> data2 = await DataBase.QueryAsync(sql2);
            var list = new List<ColumnEntity>();
            foreach (var row in data)
            {
                var model = new ColumnEntity();
