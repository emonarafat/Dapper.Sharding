...
namespace Dapper.Sharding
{
    internal class ClickHouseTableManager : ITableManager
    {
        ...
        public override Task CreateIndexAsync(string name, string columns, IndexType indexType)
        {
            throw new NotImplementedException();
        }
        ...
        public override Task DropIndexAsync(string name)
        {
            throw new NotImplementedException();
        }
        ...
        public override async Task AddColumnAsync(string name, Type t, double length = 0, string comment = null)
        {
            ...
            await DataBase.ExecuteAsync($"ALTER TABLE `{Name}` ADD COLUMN IF NOT EXISTS `{name}` {dbType} COMMENT '{comment}'");
        }
        ...
        {
            await DataBase.ExecuteAsync($"ALTER TABLE `{Name}` DROP COLUMN IF EXISTS `{name}`");
        }
        ...
        {
            var dbType = CsharpTypeToDbType.Create(DataBase.DbType, t, length);
            await DataBase.ExecuteAsync($"ALTER TABLE `{Name}` MODIFY COLUMN IF EXISTS `{name}` {dbType}");
        }
        ...
        public override Task<List<IndexEntity>> GetIndexEntityListAsync()
        {
            throw new NotImplementedException();
        }
        ...
        {
            var list = new List<ColumnEntity>();
            var columnList = await DataBase.QueryAsync($"DESCRIBE TABLE {Name}");
            foreach (var item in columnList)
            {
                var model = new ColumnEntity();
                model.Name = item.name; //列名
