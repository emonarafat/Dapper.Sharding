...
namespace Dapper.Sharding
{
    internal class MySqlTableManager : ITableManager
    {
        ...
        {
            await DataBase.ExecuteAsync($"ALTER TABLE {Name} DROP INDEX {name}");
        }
        ...
        public override async Task AddColumnAsync(string name, Type t, double length = 0, string comment = null)
        {
            ...
            await DataBase.ExecuteAsync($"ALTER TABLE `{Name}` ADD  `{name}` {dbType} COMMENT '{comment}'");
        }
        ...
        {
            var dbType = CsharpTypeToDbType.Create(DataBase.DbType, t, length);
            await DataBase.ExecuteAsync($"ALTER TABLE `{Name}` MODIFY COLUMN `{name}` {dbType} COMMENT '{comment}'");
        }
        ...
        {
            var list = new List<IndexEntity>();
            var indexList = await DataBase.QueryAsync($"SHOW INDEX FROM `{Name}`");
            var groupNames = indexList.Select(s => s.Key_name).Distinct();
            foreach (var g in groupNames)
            {
                var item = indexList.Where(f => f.Key_name == g).ToList();
        ...
        public override async Task<List<ColumnEntity>> GetColumnEntityListAsync(TableEntity tb = null)
        {
            if (tb == null)
                tb = new TableEntity();
            var list = new List<ColumnEntity>();
            var columnList = await DataBase.QueryAsync($"SHOW FULL COLUMNS FROM `{Name}`");
            foreach (var item in columnList)
            {
                ColumnEntity model = new ColumnEntity();
                model.Name = ((string)item.Field).FirstCharToUpper(); //列名
